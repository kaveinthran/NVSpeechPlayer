...
def calculatePhonemeTimes_eloquence(phonemeList,baseSpeed):
	lastPhoneme=None
	syllableStress=0
	speed=baseSpeed
...
def calculatePhonemePitches_eloquence(phonemeList,speed,basePitch,inflection,clauseType):
...
# ========== nvEspeak Mode (Post-2014 eSpeak-style) ==========

def calculatePhonemeTimes_espeak(phonemeList,baseSpeed):
	lastPhoneme=None
	syllableStress=0
	speed=baseSpeed
	for index,phoneme in enumerate(phonemeList):
...
def calculatePhonemePitches_espeak(phonemeList,speed,basePitch,inflection,clauseType):
...
def generateFramesAndTiming(ipaText,speed=1,basePitch=100,inflection=0.5,clauseType=None,mode='espeak'):
	"""
	Generate speech frames and timing for IPA text.

	Args:
		ipaText: IPA phonetic text
		speed: Speech rate
		basePitch: Base pitch frequency
		inflection: Amount of pitch variation
		clauseType: Clause type ('.', '?', '!', ',')
		mode: 'espeak' for post-2014 eSpeak-style intonation (default)
		      'eloquence' for pre-2014 Eloquence-style intonation
	"""
	...
	if mode == 'eloquence':
		calculatePhonemeTimes_eloquence(phonemeList,speed)
		calculatePhonemePitches_eloquence(phonemeList,speed,basePitch,inflection,clauseType)
	else:  # mode == 'espeak' or default
		calculatePhonemeTimes_espeak(phonemeList,speed)
		calculatePhonemePitches_espeak(phonemeList,speed,basePitch,inflection,clauseType)
